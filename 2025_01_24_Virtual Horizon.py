...
# Virtual horizon canvas creation and update logic
tk.Label(attitude_frame, text="Attitude Data", font=("Helvetica", 14, "bold")).pack()
horizon_canvas = tk.Canvas(attitude_frame, width=200, height=200, bg="black", highlightthickness=0)
horizon_canvas.pack(pady=10)

# Initialize the horizon components
horizon_background = horizon_canvas.create_rectangle(0, 0, 200, 100, fill="blue")  # Sky
horizon_ground = horizon_canvas.create_rectangle(0, 100, 200, 200, fill="green")  # Ground
horizon_line = horizon_canvas.create_line(0, 100, 200, 100, fill="white", width=2)  # Horizon line
...
# Function to update the virtual horizon based on pitch and roll
def update_virtual_horizon(pitch, roll):
    # Scale pitch and roll for the canvas
    pitch_offset = pitch * 2  # Adjust scale factor as needed
    roll_angle = roll

    # Update horizon positions
    sky_y1 = 100 - pitch_offset
    ground_y0 = 100 - pitch_offset
    horizon_canvas.coords(horizon_background, 0, sky_y1, 200, 0)
    horizon_canvas.coords(horizon_ground, 0, ground_y0, 200, 200)
    horizon_canvas.coords(horizon_line, 0, 100 - pitch_offset, 200, 100 - pitch_offset)

    # Rotate the horizon line for roll
    center_x, center_y = 100, 100 - pitch_offset
    angle_rad = roll_angle * (3.14159 / 180)  # Convert to radians
    x1 = center_x - 100 * tk.math.cos(angle_rad)
    y1 = center_y - 100 * tk.math.sin(angle_rad)
    x2 = center_x + 100 * tk.math.cos(angle_rad)
    y2 = center_y + 100 * tk.math.sin(angle_rad)
    horizon_canvas.coords(horizon_line, x1, y1, x2, y2)

# Update log_data to include virtual horizon update
def log_data():
    current_time = time.time()
    data = [current_time]
    data += [slider.get() for slider in sliders.values()]
    ...
    if msg:
        ...
        # Update attitude variables
        attitude_vars["Roll"].set(f"{roll:.1f}°")
        attitude_vars["Pitch"].set(f"{pitch:.1f}°")
        attitude_vars["Yaw"].set(f"{yaw:.1f}°")

        # Update the virtual horizon
        update_virtual_horizon(pitch, roll)
    else:
        data += [None, None, None]
    with open(log_file, mode="a", newline="") as file:
        writer = csv.writer(file)
